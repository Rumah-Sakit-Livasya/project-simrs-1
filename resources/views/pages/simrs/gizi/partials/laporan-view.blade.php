...
<body>
    ...
    <div id="functions">
        <ul>
            ...
            <li><a href="#"
                    onclick="window.location.replace('http://192.168.1.253/testing/gizi/laporan_gizi?tgl1=01-04-2025&amp;tgl2=02-05-2025&amp;norm=&amp;nama=&amp;status_makanan=&amp;xls=yes');">Xls</a>
            </li>
        ...
        <table width="100%" class="bdr2 pad">
            <thead>
                <tr>
                    ...
                    <th width="5%">Untuk</th>
                    <th width="5%">Waktu Makan</th>
                    <th width="10%">No RM / No Reg</th>
                    <th width="15%">Nama Makanan</th>
                    <th width="3%">JML</th>
                    <th width="5%">HARGA/PCS</th>
                    <th>HARGA TOTAL</th>
                    <th width="10%">% Habis</th>
            ...
            <tbody>
                ...
                @foreach ($orders as $order)
                    @php
                        ...
                        foreach ($order->foods as $food) {
                            ...
                            if (array_key_exists($id, $counted_foods)) {
                                ...
                                $counted_foods[$id]['%habis'] += $food->persentase_habis;
                            } else {
                                // if no, add the food to the array
                                $counted_foods[$id] = [
                                    'food' => $food,
                                    'quantity' => 1,
                                    '%habis' => $food->persentase_habis
                                ];
                            }
                        }
                    @endphp
                    ...
                    @foreach ($counted_foods as $food)
                        <tr>
                            ...
                            <td>{{ ucfirst($order->untuk) }}</td>
                            <td>{{ ucfirst($order->waktu_makan) }}</td>
                            ...
                            <td>
                                @php
                                    $persentase_habis = $food['%habis'] / $food['quantity'];
                                    $status = '';
                                    if ($persentase_habis <= 0) {
                                        $status = 'Tidak Disentuh (utuh)';
                                    } elseif ($persentase_habis >= 1 && $persentase_habis <= 25) {
                                        $status = 'Sisa 3/4';
                                    } elseif ($persentase_habis >= 26 && $persentase_habis <= 50) {
                                        $status = 'Sisa 1/2';
                                    } elseif ($persentase_habis >= 51 && $persentase_habis <= 75) {
                                        $status = 'Sisa 1/4';
                                    } elseif ($persentase_habis >= 76) {
                                        $status = 'Habis';
                                    }
                                @endphp
                                {{ $status }}
                ...
                <tr style="text-align: right; font-weight: bold;">
                    ...
                    <td align="right">{{ rp($orders->sum('total_harga')) }}</td>
                    <td colspan="2">&nbsp;</td>
                </tr>
            </tbody>
        </table>
