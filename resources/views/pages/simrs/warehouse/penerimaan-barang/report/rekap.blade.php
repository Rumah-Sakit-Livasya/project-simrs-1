    ...
    <div id="previews">
        ...
        <table width="100%" class="bdr2 pad">
            <thead>
                <tr>
                    <th width="3%">NO</th>
                    <th align="center" width="5%">Tgl Terima</th>
                    <th align="center" width="5%">Tgl Faktur</th>
                    <th>SUPPLIER</th>
                    <th width="10%">Kode PB</th>
                    <th width="10%">Kode PO</th>
                    <th width="10%">TIPE PENERIMAAN</th>
                    <th width="3%">TIPE PEMBAYARAN</th>
                    <th width="8%">NOMINAL</th>
                    <th width="8%">DISC</th>
                    <th width="8%">PPN(%)</th>
                    <th width="8%">MATERAI</th>
                    <th width="8%">TOTAL</th>
                </tr>
            </thead>
            <tbody>
                ...
                @foreach ($pbs as $pb)
                    <tr>
                        ...
                        <td>{{ tgl($pb->tanggal_faktur) }}</td>
                        <td>{{ $pb->supplier->nama }}</td>
                        <td>{{ $pb->kode_penerimaan }}</td>
                        <td>{{ $pb->po?->kode_po }}</td>
                        <td>{{ $pb->tipe_terima == 'po' ? 'PO' : 'Non PO' }}</td>
                        <td>{{ $pb->tipe_bayar == 'cash' ? 'Cash' : 'Non Cash' }}</td>
                        <td align="right">{{ rp($pb->total) }}</td>
                        <td align="right">{{ rp($pb->diskon_faktur) }}</td>
                        <td align="right">{{ rp($pb->ppn_nominal) }}</td>
                        <td align="right">{{ rp($pb->materai) }}</td>
                        <td align="right">{{ rp($pb->total_final) }}</td>
                    </tr>
                @endforeach

                <tr style="text-align: right; font-weight: bold;">
                    <td colspan="8">Summary</td>
                    <td>{{ rp($pbs->sum('total')) }}</td>
                    <td>{{ rp($pbs->sum('diskon_faktur')) }}</td>
                    <td>{{ rp($pbs->sum('ppn_nominal')) }}</td>
                    <td>{{ rp($pbs->sum('materai')) }}</td>
                    <td>{{ rp($pbs->sum('total_final')) }}</td>
                </tr>
            </tbody>
        </table>
