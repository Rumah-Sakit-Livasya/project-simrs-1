...
@foreach ($sis_asal as $si)
    ...
    <tr class="item pointer stock-based" id="stock{{ $si->id }}" data-type="stock"
        ondblclick="PopupSRNPharmacyClass.addItem('stock', {{ $si->id }})" data-item="{{ json_encode($item) }}">
        <td>{{ $item->kode }}</td>
...
@foreach ($items as $item)
    <tr class="item pointer barang-based" id="barang{{ $item->id }}" data-type="barang"
        ondblclick="PopupSRNPharmacyClass.addItem('barang', {{ $item->id }})" data-item="{{ json_encode($item) }}">
        <td>{{ $item->kode }}</td>
        <td class="item-name">{{ $item->nama }}</td>
        <td>
