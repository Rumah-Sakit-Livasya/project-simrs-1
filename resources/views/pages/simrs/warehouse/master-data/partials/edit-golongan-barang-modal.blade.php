<div class="modal fade" id="editModal{{ $golongan->id }}" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('warehouse.master-data.golongan-barang.update', ['id' => $golongan->id]) }}" method="post">
                @csrf
                @method('put')
                <input type="hidden" name="id" value="{{ $golongan->id }}">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="editModalLabel">Edit Golongan Barang [{{ $golongan->nama }}]</h1>
                </div>
                <div class="modal-body">
                    ...
                    <table style="width: 100%">
                        <tr>
                            ...
                            <td>:</td>
                            <td>
                                <input type="text" value="{{ $golongan->kode }}"
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td>
                                <input type="text" value="{{ $golongan->nama }}"
                                    style="border: 0; border-bottom: 1.9px solid #eaeaea; margin-top: -.5rem; border-radius: 0"
                            ...
                            <td>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="aktif" id="status_aktif_true"
                                        value="1" {{ $golongan->aktif ? 'checked' : '' }}>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="aktif"
                                        id="status_aktif_false" value="0" {{ !$golongan->aktif ? 'checked' : '' }}>
