        ...
        <div class="mb-3">
            ...
            <div id="order-notification-popup" class="card shadow border position-fixed"
                ...
                <div class="card-body p-2" id="order-notification-list" style="max-height: 350px; overflow-y: auto;">
                    ...
                    <div class="text-center text-muted py-3" id="order-notification-empty">Tidak ada notifikasi order.
                    </div>
                </div>
            </div>
        </div>
    ...
    @section('plugin-tagihan-pasien')
        <script>
            ...
            $(document).ready(function() {
                ...
                $('#tagihanTable tbody').on('click', '.delete-btn', function(e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            // Disable all delete buttons and show loading
                            ...
                            $.ajax({
                                ...
                                success: (response) => {
                                    Swal.fire('Terhapus!', 'Data tagihan telah dihapus.',
                                        'success');
                                    tagihanTable.ajax.reload(null, false);
                                },
                                error: (xhr) => {
                                    Swal.fire('Gagal!',
                                ...
                                complete: () => {
                                    $('.delete-btn').prop('disabled', false).html(
                ...
                $('#save-final, #save-draft').on('click', function() {
                    const $btn = $(this);
                    const status = $btn.attr('id') === 'save-final' ? 'final' : 'draft';
                    // Konfirmasi SweetAlert
                    Swal.fire({
                        title: `Konfirmasi`,
                        text: `Apakah Anda yakin ingin menyimpan sebagai ${status === 'final' ? 'Final' : 'Draft'}?`,
                        icon: 'question',
                        showCancelButton: true,
                        ...
                        if (result.isConfirmed) {
                            // Disable all related buttons and show loading
                            ...
                            $.ajax({
                                ...
                                error: function(xhr) {
                                    Swal.fire({
                                        ...
                                        position: 'top-end',
                                        icon: 'error',
                                        title: 'Error: ' + (xhr.responseJSON && xhr
                                            .responseJSON.error ? xhr
                                            .responseJSON.error :
                                            'Terjadi kesalahan'),
                                        showConfirmButton: false,
                                        timer: 3000
                ...
                $('#reload-tagihan').on('click', function() {
                    // Konfirmasi SweetAlert sebelum reload
                    Swal.fire({
                        title: 'Reload Tagihan',
                        text: 'Apakah Anda yakin ingin me-reload data tagihan?',
                        icon: 'question',
                        showCancelButton: true,
                        ...
                        if (result.isConfirmed) {
                            ...
                            tagihanTable.ajax.reload(null, false);
                            setTimeout(function() {
                                $('#reload-tagihan').html('Reload Tagihan');
                                $('#save-final, #save-draft, #reload-tagihan, #add-tagihan, #order-notification-btn')
                                    .prop('disabled', false);
                            }, 1000);
                        }
                ...
                $('#add-tagihan').on('click', function() {
                    // Konfirmasi SweetAlert sebelum tambah tagihan
                    Swal.fire({
                        title: 'Tambah Tagihan',
                        text: 'Apakah Anda yakin ingin menambah tagihan baru?',
                        icon: 'question',
                        showCancelButton: true,
                        ...
                        if (result.isConfirmed) {
                            ...
                            setTimeout(function() {
                                $('#add-tagihan').html('Tambah Tagihan');
                                $('#save-final, #save-draft, #reload-tagihan, #add-tagihan, #order-notification-btn')
                                    .prop('disabled', false);
                            }, 1000);
                        }
        ...
        <script>
            $(document).ready(function() {
                ...
                const notifEmpty = $('#order-notification-empty').detach();
                ...
                notifList.on('click', '.btn-process-order', function() {
                    ...
                    // Konfirmasi SweetAlert sebelum proses order
                    Swal.fire({
                        title: 'Proses Order',
                        text: 'Apakah Anda yakin ingin menambahkan order ini ke tagihan?',
                        icon: 'question',
                        showCancelButton: true,
                        ...
                        if (result.isConfirmed) {
                            // Disable all process order buttons and show loading
                            ...
                            $.ajax({
                                ...
                                error: function(xhr) {
                                    if (typeof showErrorAlertNoRefresh === 'function') {
                                        showErrorAlertNoRefresh(xhr.responseJSON && xhr
                                            .responseJSON.message ? xhr.responseJSON
                                            .message : 'Gagal memproses order.');
                                    } else {
                                        Swal.fire({
                                            icon: 'error',
                                            title: (xhr.responseJSON && xhr
                                ...
                                complete: function() {
                                    $('.btn-process-order').prop('disabled', false).html(
                                        '<i class="fal fa-plus"></i>');
                                    $('#save-final, #save-draft, #reload-tagihan, #add-tagihan, #order-notification-btn')
                                        .prop('disabled', false);
                                }
