...
@foreach ($pris as $pri)
    @if ($pri->pr) {{-- Ensure the PR is not yet deleted --}}
        <tr class="item" id="{{ $pri->id }}" data-id_pr="{{ $pri->id }}"
            data-max_qty="{{ $pri->approved_qty - $pri->ordered_qty }}" data-item="{{ json_encode($pri->barang) }}"
