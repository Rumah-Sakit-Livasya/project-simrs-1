...
@section('content')
    ...
    @include('pages.simrs.master-data.setup.biaya-materai.partials.create')
    @include('pages.simrs.master-data.setup.biaya-materai.partials.edit')
@endsection
@section('plugin')
    ...
    <script>
        $(document).ready(function() {
            let biayaMateraiId = null;
            $('#loading-spinner').show();

            $('#modal-tambah-biaya-materai .select2').select2({
                dropdownParent: $('#modal-tambah-biaya-materai')
            });

            $('#btn-tambah-biaya-materai').click(function() {
                $('#modal-tambah-biaya-materai').modal('show');
            });

            $('.btn-edit').click(function() {
                $('#modal-edit-biaya-materai').modal('show');
                biayaMateraiId = $(this).attr('data-id');
                $('#modal-edit-biaya-materai form').attr('data-id', biayaMateraiId);

                $.ajax({
                    url: '/api/simrs/master-data/setup/biaya-materai/' +
                        biayaMateraiId + '/get',
                    type: 'GET',
                    success: function(response) {
                        console.log(response);
                        $('#modal-edit-biaya-materai input[name="biaya_materai"]').val(response
                            .biaya_materai);
                        $('#modal-edit-biaya-materai input[name="min_tarif"]').val(response
                            .min_tarif);
                        $('#modal-edit-biaya-materai input[name="max_tarif"]').val(response
                            .max_tarif);
                    },
                    error: function(xhr, status, error) {
                        $('#modal-edit-biaya-materai').modal('hide');
            ...
            $('.btn-delete').click(function() {
                biayaMateraiId = $(this).attr('data-id');
                if (confirm("Apakah kamu yakin ingin melanjutkan?")) {
                    $.ajax({
                        url: '/api/simrs/master-data/setup/biaya-materai/' +
                            biayaMateraiId + '/delete',
                        type: 'DELETE',
                        success: function(response) {
            ...
            $('#store-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#modal-tambah-biaya-materai').modal('hide');
                        showSuccessAlert(response.message);
