                        ...
                        <div class="panel-content">
                            ...
                            <div class="table-responsive">
                                <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                    ...
                                    <tbody>
                                        @foreach ($kategori_radiologi as $row)
                                            <tr>
...
@section('plugin')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-tambah-kategori-radiologi').click(function() {
                $('#modal-tambah-kategori-radiologi').modal('show');
                console.log('clicked');
            });

            $('#modal-tambah-kategori-radiologi .select2').select2({
                dropdownParent: $('#modal-tambah-kategori-radiologi')
            });

            $('.btn-edit').click(function() {
                $('#modal-edit-kategori-radiologi').modal('show');
                tindakanId = $(this).attr('data-id');
                $.ajax({
                    url: '/api/simrs/master-data/penunjang-medis/radiologi/kategori/' +
                        tindakanId,
                    type: 'GET',
                    success: function(response) {
                        $('#modal-edit-kategori-radiologi input[name="nama_kategori"]').val(
                            response
                            .nama_kategori);
                        $('#modal-edit-kategori-radiologi input[name="status"][value="' +
                                response
                                .status + '"]')
                            .prop(
                                'checked', true);
                    ...
                    error: function(xhr, status, error) {
                        $('#modal-edit-kategori-radiologi').modal('hide');
            ...
            $('#store-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#modal-tambah-kategori-radiologi').modal('hide');
                        showSuccessAlert(response.message);
