...
@section('content')
                        ...
                        <div class="panel-content">
                            ...
                            <div class="table-responsive">
                                <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                    ...
                                    <tbody>
                                        @foreach ($grup_parameter_radiologi as $row)
                                            <tr>
                                                <td>{{ $row->no_urut }}</td>
                                                <td>{{ $row->nama_grup }}</td>
                                                <td>
                                        ...
                                        <tr>
                                            <th colspan="3" class="text-center">
                                                <button type="button" class="btn-outline-primary waves-effect waves-themed"
                                                    id="btn-tambah-grup-parameter-radiologi">
    ...
    @include('pages.simrs.master-data.penunjang-medis.partials.tambah-grup-parameter-radiologi')
    @include('pages.simrs.master-data.penunjang-medis.partials.edit-grup-parameter-radiologi')
@endsection
@section('plugin')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-tambah-grup-parameter-radiologi').click(function() {
                $('#modal-tambah-grup-parameter-radiologi').modal('show');
                console.log('clicked');
            });

            $('#modal-tambah-grup-parameter-radiologi .select2').select2({
                dropdownParent: $('#modal-tambah-grup-parameter-radiologi')
            });

            $('.btn-edit').click(function() {
                $('#modal-edit-grup-parameter-radiologi').modal('show');
                tindakanId = $(this).attr('data-id');
                $.ajax({
                    url: '/api/simrs/master-data/penunjang-medis/radiologi/grup-parameter-radiologi/' +
                        tindakanId,
                    type: 'GET',
                    success: function(response) {
                        $('#modal-edit-grup-parameter-radiologi input[name="no_urut"]').val(
                            response
                            .no_urut);
                        $('#modal-edit-grup-parameter-radiologi input[name="nama_grup"]').val(
                            response
                            .nama_grup);
                    },
                    error: function(xhr, status, error) {
                        $('#modal-edit-grup-parameter-radiologi').modal('hide');
            ...
            $('#store-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#modal-tambah-grup-parameter-radiologi').modal('hide');
                        showSuccessAlert(response.message);
