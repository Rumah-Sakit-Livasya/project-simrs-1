...
@section('content')
                        ...
                        <div class="panel-content">
                            ...
                            <div class="table-responsive">
                                <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                    ...
                                    <tbody>
                                        @foreach ($kategori_laboratorium as $row)
                                            <tr>
                                        ...
                                        <tr>
                                            <th colspan="3" class="text-center">
                                                <button type="button" class="btn-outline-primary waves-effect waves-themed"
                                                    id="btn-tambah-kategori-laboratorium">
    ...
    @include('pages.simrs.master-data.penunjang-medis.laboratorium.partials.tambah-kategori-lab')
    @include('pages.simrs.master-data.penunjang-medis.laboratorium.partials.edit-kategori-lab')
@endsection
@section('plugin')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-tambah-kategori-laboratorium').click(function() {
                $('#modal-tambah-kategori-laboratorium').modal('show');
            });

            $('#modal-tambah-kategori-laboratorium .select2').select2({
                dropdownParent: $('#modal-tambah-kategori-laboratorium')
            });

            $('.btn-edit').click(function() {
                console.log('clicked');
                $('#modal-edit-kategori-laboratorium').modal('show');
                kategoriId = $(this).attr('data-id');
                $('#modal-edit-kategori-laboratorium form').attr('data-id', kategoriId);

                $.ajax({
                    url: '/api/simrs/master-data/penunjang-medis/laboratorium/kategori/' +
                        kategoriId,
                    type: 'GET',
                    success: function(response) {
                        $('#modal-edit-kategori-laboratorium input[name="nama_kategori"]')
                            .val(
                                response
                                .nama_kategori);
                        $('#modal-edit-kategori-laboratorium input[name="status"][value="' +
                                response
                                .status + '"]')
                            .prop(
                                'checked', true);
                    ...
                    error: function(xhr, status, error) {
                        $('#modal-edit-kategori-laboratorium').modal('hide');
            ...
            $('#store-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#modal-tambah-kategori-laboratorium').modal('hide');
                        showSuccessAlert(response.message);
