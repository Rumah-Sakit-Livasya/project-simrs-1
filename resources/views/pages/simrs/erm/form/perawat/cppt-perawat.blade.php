                                        ...
                                        <div id="add_soap" class="panel-content collapse in" aria-expanded="true">
                                            <form method="post" class="form-horizontal" id="fsSOAP" autocomplete="off">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="card mt-3">
                                                            {{-- PERUBAHAN 1: Header Card Subjective --}}
    ...
    @if (isset($registration) && $registration->Registration_tipe === 'Rawat Inap')
Keluhan Utama: {{ $data?->keluhan_utama ?? '' }}
@else
Keluhan Utama: {{ $data?->keluhan_utama ?? '' }}
    Skor Nyeri: {{ $data?->skor_nyeri ?? '' }}
    Dokter Pemeriksa: {{ $data?->doctor?->employee?->fullname ?? '' }}
    Tanggal Pemeriksaan: {{ isset($data?->created_at)
        ? ($data->created_at instanceof \Illuminate\Support\Carbon
            ...
            : (is_string($data->created_at)
                ...
                : ''))
        : '' }}
    Riwayat Alergi: {{ isset($data) && property_exists($data, 'allergy_medicine')
