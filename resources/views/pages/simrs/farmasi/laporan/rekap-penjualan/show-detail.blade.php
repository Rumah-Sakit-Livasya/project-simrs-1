        ...
        <table width="100%" class="bdr2 pad">
            ...
            <tbody>
                ...
                @foreach ($items as $item)
                    @php
                        $isOTC = isset($item->resep->otc_id);
                    @endphp
                    <tr style="text-align: center;">
                        <td>{{ tgl($item->resep->order_date) }}</td>
                        <td style="text-align: left;">
                            {{ $isOTC ? 'OTC' : (isset($item->resep->doctor) ? $item->resep->doctor->employee->fullname : $item->resep->registration->doctor->employee->fullname) }}
                        </td>
                        <td style="text-align: left;">
                            {{ $isOTC ? $item->resep->otc->nama_pasien : $item->resep->registration->patient->name }}
                        </td>
                        <td>{{ $isOTC ? 'OTC' : $item->resep->registration->patient->medical_record_number }}</td>
                        <td>{{ $isOTC ? $item->resep->otc->registration_number : $item->resep->registration->registration_number }}
                        </td>
                        <td>{{ $item->resep->kode_resep }}</td>
