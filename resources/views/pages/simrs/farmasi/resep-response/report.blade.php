...
<body>
    ...
    <div id="previews">
        ...
        @else
            <h5>TANGGAL: {{ $endDate }}</h5>
        @endif

        <br>

        <table width="100%" class="bdr2 pad" border="1">
            <thead>
                <tr>
                    <th>No.</th>
                    <th>No. Order</th>
                    ...
                    <th>Dokter</th>
                    <th>Penjamin</th>
                    <th width="13%">Petugas Input Resep</th>
                    <th>Tgl. Input Resep</th>
                    <th width="8%">Petugas Penyiapan</th>
                    <th>Tgl. Penyiapan</th>
                    <th>Petugas Peracik</th>
                    <th>Tgl. Racik</th>
                    <th>Petugas Verifikasi</th>
                    <th>Tgl. Verifikasi</th>
                    <th>Petugas Penyerahan</th>
                    <th>Tgl. Penyerahan</th>
                    <th>Interval Waktu Penyerahan</th>
                    <th>Keterangan</th>
                </tr>
            </thead>
            <tbody>
                ...
                @foreach ($responses as $response)
                    ...
                    <tr>
                        ...
                        <td>{{ isset($response->re_id) ? $response->re->kode_re : 'NON E-RESEP' }}</p>
                        </td>
                        <td align="center">
                            {{ isset($response->re_id) ? tgl_waktu_short($response->re->created_at) : tgl_waktu_short($response->resep->created_at) }}
                        </td>
                        <td width="10%">{{ $registration->patient->name }} -
                            {{ $registration->patient->medical_record_number }}</td>
                        <td align="center">
                            @if (isset($response->resep) && isset($response->resep->doctor))
                                {{ $response->resep->doctor->department_from_doctors->name }}
                            @else
                        ...
                        <td align="center">
                            @if (isset($response->resep) && isset($response->resep->doctor))
                                {{ $response->resep->doctor->employee->fullname }}
                            @else
                                {{ $registration->doctor->employee->fullname }}
                            @endif
                        </td>
                        <td align="center">{{ $registration->penjamin->nama_perusahaan }}</td>
                        <td align="center">{{ $response->inputer?->name }}</td>
                        <td>{{ isset($response->input_resep_time) ? tgl_waktu_short($response->input_resep_time) : '' }}
                        </td>
                        <td>{{ $response->penyiap?->name }}</td>
                        <td align="center">
                            {{ isset($response->penyiapan_time) ? tgl_waktu_short($response->penyiapan_time) : '' }}
                        </td>
                        <td>{{ $response->raciker?->name }}</td>
                        <td align="center">
                            {{ isset($response->racik_time) ? tgl_waktu_short($response->racik_time) : '' }}
                        </td>
                        <td>{{ $response->verifikator?->name }}</td>
                        <td align="center">
                            {{ isset($response->verifikasi_time) ? tgl_waktu_short($response->verifikasi_time) : '' }}
                        </td>
                        <td>{{ $response->penyerah?->name }}</td>
                        <td align="center">
                            {{ isset($response->penyerahan_time) ? tgl_waktu_short($response->penyerahan_time) : '' }}
                        </td>
                        @php
                            $duration = '';
                            if (isset($response->input_resep_time) && isset($response->penyerahan_time)) {
