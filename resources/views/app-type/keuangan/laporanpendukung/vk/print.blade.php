...
<body>
    ...
    <div id="previews">
        <h2 class="bdr">LAPORAN VK</h2>
        <table width="100%" class="no-border">
            <tr>
                <td style="width: 20%;">PERIODE TGL. ORDER : </td>
                <td>{{ $tanggalAwal }} s.d. {{ $tanggalAkhir }}</td>
            </tr>
            <tr>
                <td>KELAS RAWAT : </td>
                <td>{{ $kelasRawatNama }}</td>
            </tr>
        </table>
        <table width="100%" class="pad">
            <thead>
                <tr>
                    ...
                    <th width="20%">KRU VK</th>
                    <th width="10%">HARGA</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($laporanData as $transaksi)
                    @foreach ($transaksi['kru'] as $index => $kru)
                        <tr>
                            @if ($index == 0)
                                <td class="text-center" rowspan="{{ count($transaksi['kru']) }}">
                                    {{ $transaksi['tanggal'] }}</td>
                                <td class="text-center" rowspan="{{ count($transaksi['kru']) }}">
                                    {{ $transaksi['no_rm'] }}</td>
                                <td rowspan="{{ count($transaksi['kru']) }}">{{ $transaksi['nama_pasien'] }}</td>
                                <td class="text-center" rowspan="{{ count($transaksi['kru']) }}">
                                    {{ $transaksi['kelas'] }}</td>
                                <td rowspan="{{ count($transaksi['kru']) }}">{{ $transaksi['tindakan'] ?? '' }}</td>
                            @endif
                            <td>{{ $kru['nama'] }}</td>
                            <td class="text-right">{{ number_format($kru['harga'], 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                @empty
                    <tr>
