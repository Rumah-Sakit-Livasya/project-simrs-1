...
@section('style')
    <style>
        ...
        .header-container {
            display: flex;
            align-items: flex-start; /* Ganti ke flex-start agar sejajar atas */
            margin-bottom: 20px;
            padding-bottom: 15px;
        ...
        .logo {
            width: 80px; /* Sedikit lebih kecil agar proporsional */
            height: auto;
            margin-right: 20px;
        }
        .header-info {
        ...
        .info-details { margin-bottom: 20px; font-size: 10pt; }
        .info-details ul { list-style: none; padding: 0; margin: 0; }
        .info-details li { margin-bottom: 5px; }
        .info-details li span:first-child { display: inline-block; width: 120px; font-weight: bold; }

        /* Styling untuk tabel konten */
        .content-table { width: 100%; border-collapse: collapse; }
        .content-table thead th { border: 1px solid #000; padding: 8px; background-color: #e9e9e9; font-weight: bold; text-align: center; }
        .content-table tbody td, .content-table tfoot td { border: 1px solid #000; padding: 8px; }
        .text-center { text-align: center; }
        .text-right { text-align: right; }

        /* Styling untuk checklist dokumen */
        .document-checklist { border: 1px solid #000; border-top: none; padding: 10px; ; }
        .document-checklist img { width: 12px; height: 12px; vertical-align: middle; margin-right: 5px; }

        /* Styling untuk area tanda tangan */
        .signature-area { margin-top: 40px; display: flex; justify-content: space-around; text-align: center; width: 100%; }
        .signature-box { width: 45%; }
        .signature-box .signature-name { margin-top: 70px; display: inline-block; padding-top: 5px; }
            ...
            <tbody>
                @php
                    $groupedByPO = $apSupplier->details->groupBy(function($detail) {
                        return optional(optional($detail->penerimaanBarang)->purchasable)->kode_po ?? 'NON PO';
                    });
                @endphp

                @forelse ($groupedByPO as $poCode => $details)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td class="text-center">{{ $poCode }}</td>
                        <td class="text-center">{{ $apSupplier->no_invoice_supplier }}</td>
                        <td class="text-right">{{ number_format($details->sum('nominal_grn'), 2, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="text-center">Tidak ada detail item.</td></tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr style="font-weight: bold;">
                    <td colspan="3" class="text-right">Total</td>
                    <td class="text-right">{{ number_format($apSupplier->subtotal, 2, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
