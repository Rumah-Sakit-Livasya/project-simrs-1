...
async function detectFace(video) {
    ...
    if (!response.ok) {
        console.log("Failed to load descriptors:", response.statusText);
        return;
    }
    ...
    const labeledDescriptors = employeeDescriptors
        .map((descriptor) => {
            if (
                descriptor.label &&
                Array.isArray(descriptor.descriptor) &&
                descriptor.descriptor.length === 128
            ) {
                return new faceapi.LabeledFaceDescriptors(descriptor.label, [
    ...
    if (labeledDescriptors.length === 0) {
        console.log("No valid labeled descriptors found.");
        return;
    }
        ...
        if (detections.length > 0) {
            ...
            results.forEach(async (result, i) => {
                if (result.distance < 0.6) {
                    ...
                    console.log(
                        `Face matched with employee ID: ${result.label}`
                    );
