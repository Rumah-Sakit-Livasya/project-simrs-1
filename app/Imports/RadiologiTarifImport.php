...
// SEMUA 'use' statement HARUS ADA DI SINI, SEBELUM 'class'
...
class RadiologiTarifImport implements ToCollection, WithStartRow, WithEvents
{
    ...
    {
        $lastParameter = ParameterRadiologi::withTrashed()
            ->where('kode', 'LIKE', 'RAD%')
            ->orderBy('id', 'desc')
            ->first();
        ...
        DB::transaction(function () use ($rows, &$lastCodeNumber) {
            foreach ($rows as $rowIndex => $row) {
                ...
                $grup = GrupParameterRadiologi::firstOrCreate(['nama_grup' => $groupName], ['no_urut' => 0]);
                $kategori = KategoriRadiologi::firstOrCreate(['nama_kategori' => $kategoriName], ['status' => 1]);
                ...
                if (!empty($idFromCell)) {
                    ...
                    $parameter = ParameterRadiologi::find($idFromCell);

                    if ($parameter) {
                        // DITEMUKAN: Lakukan UPDATE
                        Log::info("--> Ditemukan. Mengupdate '{$parameter->parameter}' menjadi '{$parameterName}'.");
                        $parameter->update([
                            'grup_parameter_radiologi_id' => $grup->id,
                            'kategori_radiologi_id' => $kategori->id,
                    ...
                    $parameter = ParameterRadiologi::firstOrCreate(
                        ['parameter' => $parameterName, 'grup_parameter_radiologi_id' => $grup->id],
                        [
                ...
                if ($parameter) {
                    TarifParameterRadiologi::where('parameter_radiologi_id', $parameter->id)
                    ...
                    foreach ($this->kelasRawatMap as $kelas) {
                        ...
                        if ($nilai_tarif !== null && $nilai_tarif !== '' && (float)$nilai_tarif > 0) {
                            TarifParameterRadiologi::create([
