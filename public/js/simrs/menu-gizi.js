...
class ModalMenuGiziHandler {
    ...
    #updateHarga(change) {
        ...
        this.#FormElement.find('#harga-display').text(`Rp ${hargaTotal.toLocaleString('id-ID')}`);
    }
    ...
    #getFoodTableCol(food) {
        ...
        return /*html*/`
            <tr id="food${key}">
                ...
                <td>
                    <input type="hidden" name="foods_id[${key}]" value="${food.id}">
                    <input type="checkbox" name="foods_status[${key}]" value="1" checked
                        onchange="window['handler_${this.#FormSelector}'].updateFoodStatus(this.checked, ${food.harga})">
                </td>
                <td>
                    <a class="mdi mdi-close pointer mdi-24px text-danger delete-btn"
                        title="Hapus" onclick="window['handler_${this.#FormSelector}'].deleteFood(${key}, ${food.harga})"></a>
                </td>
            </tr>
        `;
    }
    ...
    */
    ...
    deleteFood(key, change) {
        const isChecked = this.#FormElement.find(`input[name="foods_status[${key}]"]`).is(':checked');
        if (isChecked) {
            this.#updateHarga(-change);
        }
