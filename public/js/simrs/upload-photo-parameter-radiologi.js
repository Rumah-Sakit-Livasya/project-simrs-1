...
class UploadPhotoParameterRadiologi {
    ...
    handleUploadButtonClick(event) {
        ...
        const fileInput = /** @type {HTMLInputElement} */ (
            this.#querySelectorAbs('input[name="photo[]"].input-' + parameterId)
        );
        ...
        const CSRF =
            this.#querySelectorAbs('meta[name="csrf-token"]').getAttribute(
                "content"
            ) || "";
        const userId = /** @type {HTMLInputElement} */ (
            this.#querySelectorAbs('[name="user_id"].input-' + parameterId)
        ).value;
        const employeeId = /** @type {HTMLInputElement} */ (
            this.#querySelectorAbs('[name="employee_id"].input-' + parameterId)
        ).value;
