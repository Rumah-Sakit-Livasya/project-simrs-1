// public/js/simrs/modules/tindakan-medis.js

// Membuat namespace global untuk modul-modul SIMRS jika belum ada
window.SimrsModules = window.SimrsModules || {};

SimrsModules.TindakanMedis = (function ($) {
    // Variabel privat untuk modul ini
    let currentIndex = 1;

    // Fungsi privat
    function addMedicalActionToTable(data) {
        const doctorName = data.doctor?.employee?.fullname || "Tidak Diketahui";
        const actionName =
            data.tindakan_medis?.nama_tindakan || "Tidak Diketahui";
        ...
        const newRow = `
            <tr>
                ...
                <td style="white-space: nowrap;">${
                    data.tanggal_tindakan || "Tidak Diketahui"
                }</td>
                <td>${doctorName}</td>
                <td>${actionName}</td>
                <td>${className}</td>
                ...
                <td>${foc}</td>
                <td>
                    <button class="btn btn-danger btn-sm delete-action" data-id="${
                        data.id
                    }">Hapus</button>
                </td>
            </tr>
    ...
    // Fungsi yang akan dipanggil untuk menginisialisasi modul
    function init() {
        // Cek apakah elemen yang relevan ada di halaman, jika tidak, jangan jalankan apapun.
        if ($("#tindakan-medis").length === 0) {
            return;
        }

        console.log("Modul Tindakan Medis diinisialisasi.");

        // Pindahkan semua event listener dan logika Anda ke sini
        $("#tindakan-medis").hide();

        $('.menu-layanan[data-layanan="tindakan-medis"]').on(
            "click",
            function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.success) {
                            ...
                            data.forEach((action) => {
                                const doctorName =
                                    action.doctor?.employee?.fullname ||
                                    "Tidak Diketahui";
                                const actionName =
                                    action.tindakan_medis?.nama_tindakan ||
                                    "Tidak Diketahui";
                                const className =
                                    action.departement?.name ||
                                    "Tidak Diketahui";
                                const qty = action.qty || 0;
                                const userName =
                                    action.user?.employee?.fullname ||
                                    "Tidak Diketahui";
                            ...
                            <tr>
                                <td>${currentIndex++}</td>
                                <td style="white-space: nowrap;">${
                                    action.tanggal_tindakan || "Tidak Diketahui"
                                }</td>
                                <td>${doctorName}</td>
                                <td>${actionName}</td>
                                <td>${className}</td>
                    ...
                    error: function (xhr) {
                        ...
                        if (xhr.responseJSON && xhr.responseJSON.message) {
                            ...
                            errorMessage = `Gagal memuat tindakan medis. Status: ${xhr.status}, Pesan: ${xhr.statusText}`;
                        }
                    ...
                    $.ajax({
                        ...
                        error: function (xhr) {
                            ...
                            if (xhr.responseJSON && xhr.responseJSON.message) {
                                ...
                                errorMessage = `Gagal menghapus tindakan medis. Status: ${xhr.status}, Pesan: ${xhr.statusText}`;
                            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        modal.modal("hide");
                        showSuccessAlert(
                            "Tindakan medis berhasil ditambahkan!"
                        );
                    } else {
                        showErrorAlertNoRefresh(
                            "Gagal menambahkan tindakan medis: " +
                                response.message
                        );
                    }
                },
    ...
    // "Return" objek publik yang bisa diakses dari luar
    return {
        init: init,
    };
})(jQuery); // Pastikan jQuery di-passing
