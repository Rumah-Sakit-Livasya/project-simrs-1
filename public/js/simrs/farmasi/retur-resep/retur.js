...
class ReturResepHandler {
    ...
    #handleItemSearchBar(event) {
        ...
        items.forEach((item) => {
            ...
            if (kode_barang.toLowerCase().includes(ItemName) ||
    ...
    #getItemTableCol(item) {
        ...
        return /*html*/`
            <tr id="item${key}" class="fri${item.id}">
                ...
                <input type="hidden" name="subtotal[${key}]" value="${item.harga}">
                ...
                <td><input type="number" name="qty[${key}]" class="form-control" value="1" min="1" max="${item.qty}"></td>
