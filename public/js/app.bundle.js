...
var myapp_config = {
	...
	*/
    ...
    ), //popular device types available on the market
	...
	*/
    isWebkit:
        (!!window.chrome && !!window.chrome.webstore) === true ||
        ...
        navigationDestroy: function () {
            ...
            if (self.hasClass(myapp_config.navInitalized)) {
                ...
                self.find("li a")
                    ...
                    .find(".collapse-sign")
                    .remove();
                self.removeClass(myapp_config.navInitalized)
                    .find("ul")
                    .removeAttr("style");
    ...
    function Plugin(element, options) {
        ...
        function init() {
            ...
            $el.wrap(
                '<div id="' +
                    options.wrapperId +
                    '" class="nav-menu-wrapper d-flex flex-grow-1 width-0 overflow-hidden"></div>'
            );

            /* add buttons for scroller */
            $("#" + options.wrapperId).before(
                '<a href="#" id="' +
                    options.wrapperId +
                    '-left-btn" class="d-flex align-items-center justify-content-center width-4 btn mt-1 mb-1 mr-2 ml-1 p-0 fs-xxl text-primary"><i class="fal fa-angle-left"></i></a>'
            );
            $("#" + options.wrapperId).after(
                '<a href="#" id="' +
                    options.wrapperId +
                    '-right-btn" class="d-flex align-items-center justify-content-center width-4 btn mt-1 mb-1 mr-1 ml-2 p-0 fs-xxl text-primary"><i class="fal fa-angle-right"></i></a>'
            );

            var getListWidth = $.map(
                    $el.children("li:not(.nav-title)"),
                    function (val) {
                ...
                _getValues = function () {
                    wrapperWidth = $(
                        "#" + options.wrapperId
                    ).outerWidth(); /* incase its changed we get it again */
                    contentWidth = $.map(
                        $el.children("li:not(.nav-title)"),
                        function (val) {
                            return $(val).outerWidth(true);
                ...
                navMenuScrollRight = function () {
                    ...
                    if (-currentMarginLeft + wrapperWidth < contentWidth) {
                        setMargin = Math.max(
                            currentMarginLeft - wrapperWidth,
...
var initApp = (function (app) {
                ...
                if (filter.length > 1) {
				   ...
				   hiding the ones not containing the input while showing the ones that do */
                    ...
                    if (listPrev) {
                        listPrev.text(
                            "showing " +
                                $(list).find("li.js-filter-show").length +
                                " from " +
                                $(list).find("[data-filter-tags]").length +
                                " total"
                        );
                    }
                } else {
    ...
    app.resetSettings = function () {
        ...
        myapp_config.root_.removeClass(function (index, className) {
            return (
                className.match(/(^|\s)(nav-|header-|mod-|display-)\S+/g) || []
            ).join(" ");
        });
        ...
        if (myapp_config.debugState) console.log("App reset successful");
    };
    ...
    app.pushSettings = function (DB_string, saveToLocal) {
        ...
        if (saveToLocal != false) localStorage.setItem("themeSettings", "");
        ...
        if (saveToLocal != false) initApp.saveSettings();
    ...
    app.playSound = function (path, sound) {
        ...
        else audioElement.setAttribute("src", path + "/" + sound + ".mp3");
    ...
    app.checkNavigationOrientation = function () {
        ...
        switch (true) {
            case myapp_config.root_.hasClass("nav-function-fixed") &&
                !myapp_config.root_.is(
                    ".nav-function-top, .nav-function-minify, .mod-main-boxed"
                ) &&
                myapp_config.thisDevice === "desktop":
                ...
                if (typeof $.fn.slimScroll !== "undefined") {
                    ...
                    if (
                        document.getElementById(
                            myapp_config.navHorizontalWrapperId
                        )
                    ) {
            ...
            case myapp_config.navAnchor.parent().hasClass("slimScrollDiv") &&
                myapp_config.thisDevice === "desktop" &&
        ...
        switch (true) {
            ...
            case $.fn.menuSlider &&
                myapp_config.root_.hasClass("nav-function-top") &&
                $("#js-nav-menu-wrapper").length == false &&
                !myapp_config.root_.hasClass("mobile-view-activated"):
            ...
            case myapp_config.root_.hasClass("nav-function-top") &&
                $("#js-nav-menu-wrapper").length == true &&
                myapp_config.root_.hasClass("mobile-view-activated"):
    ...
    app.domReadyMisc = function () {
         ...
         **/
        if (
            typeof initApp.listFilter !== "undefined" &&
            $.isFunction(initApp.listFilter) &&
        ...
        myapp_config.root_.on("click touchend", "[data-action]", function (e) {
            ...
            switch (true) {
                ...
                case actiontype === "toggle":
                    var target =
                            $(this).attr("data-target") || myapp_config.root_,
...
jQuery(document).ready(function () {
     ...
     **/
    initApp.listFilter(
        myapp_config.navHooks,
        myapp_config.navFilterInput,
...
$(window).on("orientationchange", function (event) {
    ...
    if (myapp_config.debugState) console.log("orientationchange event");
});
    ...
    if (prevType != e.type) {
        switch (e.type) {
            case "blur":
                ...
                if (myapp_config.debugState) console.log("blur");

                break;
            case "focus":
                myapp_config.root_.toggleClass("blur");
                if (myapp_config.debugState) console.log("focused");
