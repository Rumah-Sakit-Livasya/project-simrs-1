...
(function (root) {
    ...
    sync = {
                ...
                f.each(lngs, function (key, lng) {
                    ...
                    if (local) {
                        ...
                        if (
                            local.i18nStamp &&
                            local.i18nStamp +
                                options.localStorageExpirationTime >
                                nowMS
                        ) {
                            store[lng] = local;
                        }
                ...
                if (typeof options.customLoad == "function") {
                    ...
                    f.ajax({
                        ...
                        error: function (xhr, status, error) {
                            ...
                            loadComplete(
                                "failed loading resource.json error: " + error
                            );
                        },
                        dataType: "json",
        ...
        postMissing: function (lng, ns, key, defaultValue, lngs) {
            ...
            if (o.sendMissingTo === "fallback" && o.fallbackLng[0] !== false) {
                ...
                o.sendMissingTo === "current" ||
                (o.sendMissingTo === "fallback" && o.fallbackLng[0] === false)
            ) {
                urls.push({
                    lng: lng,
                    url: applyReplacement(o.resPostPath, { lng: lng, ns: ns }),
                });
            ...
            for (var y = 0, len = urls.length; y < len; y++) {
                ...
                f.ajax({
                    ...
                    success: function (data, status, xhr) {
                        f.log(
                            "posted missing key '" + key + "' to: " + item.url
                        );
    ...
    var f = {
        ...
        toLanguages: function (lng, fallbackLng) {
            ...
            if (typeof lng === "string" && lng.indexOf("-") > -1) {
                ...
                if (o.load !== "current")
                    addLanguage(
                        applyCase(parts[this.getCountyIndexOfLng(lng)])
                    );
            } else {
    ...
    function init(options, cb) {
        ...
        // fallback languages
        if (
            typeof o.fallbackLng == "string" ||
            typeof o.fallbackLng == "boolean"
        ) {
    ...
    function addJqueryFunct() {
        ...
        function parse(ele, key, options) {
            ...
            if (attr === "html") {
                ...
                optionsToUse = o.defaultValueFromContent
                    ? $.extend({ defaultValue: ele.data(dataAttr) }, options)
                    : options;
    ...
    function applyReplacement(str, replacementHash, nestedKey, options) {
        ...
        if (
            str.indexOf(options.interpolationPrefix || o.interpolationPrefix) <
            0
        )
            return str;
        ...
        var hash =
            replacementHash.replace &&
            typeof replacementHash.replace === "object"
                ? replacementHash.replace
                : replacementHash;
        var replacementRegex = new RegExp(
            [prefix, "(.+?)", "(HTML)?", suffix].join(""),
            "g"
        );
        var escapeInterpolation =
            options.escapeInterpolation || o.escapeInterpolation;
        return str.replace(
            replacementRegex,
            function (wholeMatch, keyMatch, htmlMatched) {
                // Check for recursive matches of object
                var objectMatching = hash;
                var keyLeaf = keyMatch;
                while (
                    keyLeaf.indexOf(keyseparator) >= 0 &&
                    typeof objectMatching === "object" &&
                    objectMatching
                ) {
                    var propName = keyLeaf.slice(
                        0,
                        keyLeaf.indexOf(keyseparator)
                    );
                ...
                if (
                    objectMatching &&
                    typeof objectMatching === "object" &&
                    objectMatching.hasOwnProperty(keyLeaf)
                ) {
    ...
    function applyReuse(translated, options) {
        ...
        while (translated.indexOf(o.reusePrefix) != -1) {
            ...
            var token_without_symbols = token
            ...
            if (token_without_symbols.indexOf(comma) != -1) {
                var index_of_token_end_of_closing =
                    token_without_symbols.indexOf(comma);
                if (
                    token_without_symbols.indexOf(
                        options_open,
                    ...
                    var index_of_opts_opening = token_without_symbols.indexOf(
                        options_open,
                        index_of_token_end_of_closing
                    );
                    var index_of_opts_end_of_closing =
                        token_without_symbols.indexOf(
                            options_close,
                            index_of_opts_opening
                        ) + options_close.length;
                    try {
                        opts = f.extend(
                            opts,
                            JSON.parse(
                                token_without_symbols.substring(
                                    index_of_opts_opening,
                                    index_of_opts_end_of_closing
                                )
                            )
                        );
                        token_without_symbols = token_without_symbols.substring(
                            0,
                            index_of_token_end_of_closing
                        );
                    } catch (e) {}
            ...
            translated = translated.replace(
                token,
                f.regexReplacementEscape(translated_token)
            );
        }
        return translated;
    }
    ...
    function needsPlural(options, lng) {
        return (
            ...
            typeof options.count !=
                "string" /* && pluralExtensions.needsPlural(lng, options.count)*/
        );
    }

    function needsIndefiniteArticle(options) {
        return (
            options.indefinite_article !== undefined &&
            typeof options.indefinite_article != "string" &&
    ...
    function _translate(potentialKeys, options) {
        ...
        var notFound = _getDefaultValue(key, options),
            ...
            lngs = options.lng
                ? f.toLanguages(options.lng, options.fallbackLng)
                : languages,
        ...
        if (
            found === undefined &&
            o.sendMissing &&
            typeof o.missingKeyHandler === "function"
        ) {
            if (options.lng) {
        ...
        if (typeof o.postProcess === "string" && o.postProcess !== "") {
            ...
            typeof o.postProcess === "array" ||
            typeof o.postProcess === "object"
        ) {
            postProcessorsToApply = o.postProcess;
        } else {
        ...
        if (
            typeof options.postProcess === "string" &&
            options.postProcess !== ""
        ) {
            postProcessorsToApply = postProcessorsToApply.concat([
                options.postProcess,
            ]);
        } else if (
            typeof options.postProcess === "array" ||
        ...
        if (found === undefined) {
            ...
            if (postProcessorsToApply.length) {
                ...
                for (j = 0; j < postProcessorsToApply.length; j += 1) {
                    postProcessor = postProcessorsToApply[j];
                    if (postProcessors[postProcessor]) {
                        found = postProcessors[postProcessor](
                            found,
                            key,
                            options
                        );
    ...
    function _find(key, options) {
        ...
        if (needsPlural(options, lngs[0])) {
            ...
            optionWithoutCount._origLng =
                optionWithoutCount._origLng ||
                optionWithoutCount.lng ||
                lngs[0];
            ...
            if (!pluralExtensions.needsPlural(lngs[0], options.count)) {
                ...
                var pluralExtension = pluralExtensions.get(
                    lngs[0],
                    options.count
                );
        ...
        if (needsIndefiniteArticle(options)) {
            ...
            var indefiniteKey =
                ...
                nsseparator +
                key +
                ((options.count && !needsPlural(options, lngs[0])) ||
                !options.count
                    ? o.indefiniteSuffix
                    : "");
            ...
            if (value !== undefined && (!o.showKeyIfEmpty || value !== "")) {
                ...
                if (typeof value === "string") {
                    ...
                    if (!o.returnObjectTrees && !options.returnObjectTrees) {
                        if (
                            o.objectTreeKeyHandler &&
                            typeof o.objectTreeKeyHandler == "function"
                        ) {
                            value = o.objectTreeKeyHandler(
                                key,
                                value,
                                l,
                                ns,
                        ...
                        valueType !== "[object RegExp]"
                    ) {
                        var copy = valueType === "[object Array]" ? [] : {}; // apply child translation on a copy
                        f.each(value, function (m) {
                            copy[m] = _translate(
                                ns + nsseparator + key + keyseparator + m,
                                options
                            );
                        });
                        value = copy;
        ...
        if (
            ...
            if (o.fallbackNS.length) {
                for (var y = 0, lenY = o.fallbackNS.length; y < lenY; y++) {
                    ...
                    if (
                        found ||
                        (found === "" && o.fallbackOnEmpty === false)
                    ) {
                        /* compare value without namespace */
                        var foundValue =
                                found.indexOf(nsseparator) > -1
                                    ? found.split(nsseparator)[1]
                                    : found,
                            notFoundValue =
                                notFound.indexOf(nsseparator) > -1
                                    ? notFound.split(nsseparator)[1]
                                    : notFound;
    ...
    addPostProcessor("sprintf", function (val, key, opts) {
        ...
        if (
            Object.prototype.toString.apply(opts.sprintf) === "[object Array]"
        ) {
